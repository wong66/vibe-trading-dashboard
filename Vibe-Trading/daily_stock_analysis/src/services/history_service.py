...
class HistoryService:
    ...
    def get_news_intel(self, query_id: str, limit: int = 20) -> List[Dict[str, str]]:
        ...
        try:
            ...
            # 第三层兜底：当内部新闻情报（含历史同股票兜底）均为空时，
            # 回退调用 VT 主后端的 /stock-reports（东财+爱问财研报）填充资讯区。
            if not items:
                vt_items = self._fallback_news_from_vt(query_id=query_id, limit=limit)
                if vt_items:
                    return vt_items
    ...
    def _fallback_news_from_vt(self, query_id: str, limit: int) -> List[Dict[str, str]]:
        """
        第三层兜底：调用 VT 主后端 /stock-reports 获取该股票近期研报。

        仅在以下场景触发：
        - news_intel 表无该 query_id 的记录；
        - 同股票历史新闻兜底也为空；
        - 且配置了 VT_STOCK_REPORTS_FALLBACK 不为 0/false/no/off。

        使用标准库 urllib（避免新增依赖），并强制不走代理以确保本地回环可达。
        """
        import os
        import re
        import json
        ...
        try:
            enabled = os.getenv("VT_STOCK_REPORTS_FALLBACK", "1").strip().lower()
            if enabled in ("0", "false", "no", "off"):
                return []
            base = os.getenv("VT_STOCK_REPORTS_BASE_URL", "http://127.0.0.1:8898").strip().rstrip("/")
            if not base:
                return []

            recs = self.db.get_analysis_history(query_id=query_id, limit=1)
            if not recs or not getattr(recs[0], "code", None):
                return []
            raw_code = str(recs[0].code).strip()
            # 归一化为纯数字代码（去掉 .SH/.SZ/.BJ 或 SH 前缀），适配东财/爱问财接口
            m = re.search(r"\d{4,6}", raw_code)
            code = m.group(0) if m else raw_code
            # 仅对纯数字 A 股/港股代码调用研报接口；MARKET/BENCHMARK 等非个股代码跳过
            if not code or not code.isdigit():
                return []

            url = f"{base}/stock-reports?code={urllib.parse.quote(code)}&months=6"
            req = urllib.request.Request(
                url,
                headers={"User-Agent": "DSA-NewsFallback/1.0", "Accept": "application/json"},
            )
            # 强制绕过系统代理，保证本地回环请求直达 VT 后端
            ...
            for r in reports[:limit]:
                ...
                meta_parts = [p for p in (org, date, f"评级{rating}" if rating else "") if p]
                meta = " · ".join(meta_parts)
                snippet = summary if summary else meta
                if not snippet and source:
                    snippet = f"来源：{source}"
                if len(snippet) > 200:
                    snippet = f"{snippet[:197]}..."
                items.append({
            ...
            if items:
                logger.info(
                    "VT /stock-reports 兜底命中 %d 条研报 (query_id=%s, code=%s)",
                    len(items), query_id, code,
                )
            return items

        except urllib.error.HTTPError as e:
            logger.warning("VT /stock-reports 兜底 HTTP 错误 %s: %s", getattr(e, "code", ""), e)
            return []
        except Exception as e:  # noqa: BLE001
            logger.warning("VT /stock-reports 兜底失败 (query_id=%s): %s", query_id, e)
            return []
